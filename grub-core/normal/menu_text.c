...
print_message (int nested, int edit, struct grub_term_output *term, int dry_run)
{
  ...
  if (edit)
    {
      ret += grub_print_message_indented_real (_("Minimum Emacs-like editing is \
supported. TAB lists completions. Press Ctrl-x or F10 to boot, Ctrl-c or F2 for a \
command-line or ESC to discard edits and return to the V-OS Bootloader menu."),
      ...
      if (nested)
	{
	  ret += grub_print_message_indented_real
	    (_("Press enter to boot the OS, "
	       "`e' to edit the boot flags before booting "
